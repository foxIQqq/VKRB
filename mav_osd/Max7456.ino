...
void MAX7456_SetScreenSymbol(char symbol, uint16_t position)
{
  screen[position] = symbol;
  screen_update_mask[position >> 3] |= (1 << (position & 0x7));
}
...

{
  ...
  uint8_t curMaskByte = addr / 8;
  uint8_t curMaskBit = addr % 8;
  uint8_t curMask = screen_update_mask[curMaskByte];
  while (*string) {
    // MAX7456_SetScreenSymbol(*string++, addr);
    *screenp++ = *string++;
    curMask |= (1 << curMaskBit);
    if (++curMaskBit == 8) {
      screen_update_mask[curMaskByte++] = curMask;
      curMask = screen_update_mask[curMaskByte];
      curMaskBit = 0;
    }
  }
  screen_update_mask[curMaskByte] = curMask;
...
void MAX7456_WriteString_P(const char *string, int Adresse)
{
  ...
  uint8_t curMaskByte = Adresse / 8;
  uint8_t curMaskBit = Adresse % 8;
  uint8_t curMask = screen_update_mask[curMaskByte];
  while((c = (char)pgm_read_byte(string++)) != 0) {
    // MAX7456_SetScreenSymbol(c, Adresse++);
    *screenp++ = c;
    curMask |= (1 << curMaskBit);
    if (++curMaskBit == 8) {
      screen_update_mask[curMaskByte++] = curMask;
...
void MAX7456_ClearScreen(void)
{
  for(uint16_t xx = 0; xx < MAX_screen_size; ++xx) {
    screen[xx] = ' ';
  for(uint16_t xx = 0; xx < sizeof(screen_update_mask_prev); ++xx)
    screen_update_mask_prev[xx] = 0xFF;
  }
}
#endif
...
void MAX7456_DrawScreen() {
  ...
  for(uint8_t curMaskByte = 0; curMaskByte < sizeof(screen_update_mask); ++curMaskByte) {
    if ((curMaskByte << 3) >= MAX_screen_size)
      break;
    
    uint8_t curMask = screen_update_mask[curMaskByte];
    uint8_t curMaskPrev = screen_update_mask_prev[curMaskByte];
    if ((curMask == 0) && (curMaskPrev == 0))
      continue;

    bool firstBitInByte = true;
    for (uint8_t curMaskBit = 0; curMaskBit < 8; ++curMaskBit) {
      if ((curMask & (1 << curMaskBit)) || (curMaskPrev & (1 << curMaskBit))) {
        uint16_t position = (curMaskByte << 3) + curMaskBit;
        char *screen_address = screen + position;
        
        if (firstBitInByte) {
          MAX7456ENABLE
          MAX7456_Send(MAX7456ADD_DMAH, position >> 8);
          firstBitInByte = false;
        }
        MAX7456_Send(MAX7456ADD_DMAL, position & 0xFF);
        if (curMask & (1 << curMaskBit))
          MAX7456_Send(MAX7456ADD_DMDI, *screen_address);
        else
          MAX7456_Send(MAX7456ADD_DMDI, ' ');
      }
    }
    if (!firstBitInByte) MAX7456DISABLE;
  }

  memcpy(screen_update_mask_prev, screen_update_mask, sizeof(screen_update_mask));
  memset(screen_update_mask, 0, sizeof(screen_update_mask));
